...
private extension NewObjectViewController {
    ...
    func makeConstraints() {
        ...
        typeTF.snp.makeConstraints {
            $0.height.equalTo(nameTF.snp.height)
            $0.width.equalTo(nameTF.snp.width)
        }
    ...
    @objc func goBack() {
        let patternName = nameTF.text
        let patternDescription = descriptionTF.text ?? "No description"
        let pattern = Model(
            name: patternName!,
...
extension NewObjectViewController: UITextFieldDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        nameTF.endEditing(false)
        descriptionTF.endEditing(false)
        return false
    }
}
